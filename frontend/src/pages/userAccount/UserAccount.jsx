...
const UserAccount = () => {
  ...
  const getUserDetails = async () => {
    ...
    const feedback = await axios.get(
      `${import.meta.env.VITE_BACKEND_URL}/get-user-orders`,
      {
  ...
  useEffect(() => {
    ...
    getUserDetails();
  }, [use_auth.loading, currentPage]);

  if (use_auth.loading) return null;
              ...
              <>
                {
                  orderList.map(order => (
                    <div className="order-card" key={order.id}>
                      ...
                      <div>
                        {currencySymbols[selectedCurrency]}
                        {convertCurrency(order.total_amount)}
                      </div>
                ...
                {
                  totalOrders?.last_page > 1 &&
                  <PaginationButtons
                    currentPage={currentPage}
                    totalPages={totalOrders.last_page}
